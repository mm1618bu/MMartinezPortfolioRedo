...
interface DraggableAssignmentCellProps {
...
export const DraggableAssignmentCell: React.FC<DraggableAssignmentCellProps> = ({
  assignment,
  employeeId,
  date,
  onSelect,
  ...
  const isDraggingThis = dragState.draggedAssignment?.id === assignment.id;

  const handleDragStart = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      e.stopPropagation();
      startDrag(assignment, employeeId, date);

      // Set drag image (default browser image)
    ...
    [assignment, employeeId, date, startDrag]
  );
  ...
  return (
    <div
      ...
      } ${isSelected ? 'selected' : ''} ${isDraggingThis ? 'dragging' : ''}`}
      onClick={handleClick}
    ...
    >
      ...
      <div className="cell-time">{formatTime(assignment.shift_start_time)}</div>
...
export default DraggableAssignmentCell;
